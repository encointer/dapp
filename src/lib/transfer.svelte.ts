...
async function buildHopTx(
  ...
  // Cross-chain via paraspell. (paraspell ≥ 13 fixes the KAH→PAH USDC
  // sub-bridge reserve detection that earlier versions mis-routed.)
  const overrides = getApiOverrides()
  if (!overrides) throw new Error('Not connected to chains')
  const currency = { ...getCurrency(hop.from, token), amount: amount }
  ...
  try {
    ...
    for (const hop of route.hops) {
      ...
      if (hop.from === hop.to) {
        ...
        const currency = { ...getCurrency(hop.from, params.token), amount: params.amount }
